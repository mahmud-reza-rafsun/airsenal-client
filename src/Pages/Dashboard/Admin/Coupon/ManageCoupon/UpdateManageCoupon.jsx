...
const UpdateManageCoupon = () => {
    const axiosSecure = useAxiosSecure();
    const { id } = useParams();
    const { data: couponData = {} } = useQuery({
        queryKey: ['couponData', id],
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/get-coupon/${id}`);
            return data;
        }
    })
    ...
    const handleAddProduct = async (e) => {
        ...
        try {
            await axiosSecure.patch(`/update-coupon/${id}`, couponData);
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-2xl mx-auto'>
                    ...
                    <div className='space-y-1 text-sm'>
                        ...
                        <input
                            ...
                            id='name'
                            type='text'
                            defaultValue={couponData?.ammount}
                            placeholder='Ammount'
                            required
                        />
                    </div>
...
export default UpdateManageCoupon;